...
PRD 기반 텔레그램 **출석체크 자동화 봇**입니다. 매주 일요일 정해진 시간에 출석 세션을 자동으로 열고, `/attend` 명령(또는 버튼 클릭)으로 출석을 기록하며, 출석 현황 메시지를 **edit(수정)** 으로 계속 갱신합니다.
...
- Python **3.10+**
- 텔레그램 봇 토큰 (`BOT_TOKEN`)
- 출석을 운영할 단체방 Chat ID (`GROUP_CHAT_ID`, 보통 `-100...` 형태)
...
- 세션 오픈 시:
  - 오픈 안내 메시지 1개 전송
  - 출석 현황 메시지 1개 전송(인라인 `/attend` 버튼 포함)
- 출석이 들어올 때마다:
